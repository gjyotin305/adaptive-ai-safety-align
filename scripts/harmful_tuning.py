...
class SFT_trainer_w_kl(SFTTrainer):
    def __init__(self, ref_model_name, template_name, *args, **kwargs): 
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        score_harm = get_stream_judge_response(inputs, self.processing_class, template=self.template)
...
def main(
    ...
    method: Annotated[str, typer.Option("--method", "-mt")] = "old"
):
    max_seq_length = 2048
    dtype = None
    load_in_4bit = False
    ...
    # list_model_names = [
    #     # ("unsloth/Qwen2.5-3B-Instruct", "qwen"),
    #     # ("unsloth/Qwen2.5-7B-Instruct", "qwen"),
    #     # ("unsloth/Llama-3.2-3B-Instruct", "llama"),
    #     # ("unsloth/Meta-Llama-3.1-8B-Instruct", "llama"),
    #     # ("unsloth/Phi-3.5-mini-instruct", "phi")
    #     # ("unsloth/gemma-3-1b-it", "gemma")
    # ]
    ...
    # train_text = {}
    # train_text['text'] = tokenizer.apply_chat_template(train_dataset['text'], tokenize=False)
    # train_dataset_final = Dataset.from_dict(train_text)
    if method == "new":
        trainer = SFT_trainer_w_kl(
            model = model,
